...
#define _PULSE_COMMON_INCLUDED
    ...
    // Sample blit texture with minimum amount code line

    float4 sampleBlit(float2 uv)
    {
        return SAMPLE_TEXTURE2D_X(_BlitTexture, _sampler_Linear_Clamp, uv);
    }

    half4 sampleBlitLod(float2 uv, float lod)
    {
        return SAMPLE_TEXTURE2D_LOD(_BlitTexture, _sampler_Linear_Clamp, uv, lod);
    }

    float sampleDepthLod(float2 uv, float lod)
    {
        return SAMPLE_TEXTURE2D_LOD(_CameraDepthTexture, _sampler_Linear_Clamp, uv, lod).x;
    }
