...
half2 SpatialGTAO_Y_frag(PixelInput IN) : SV_Target
{
    ...
    half3 worldNormal = tex2D(_GBuffer2, uv).rgb * 2 - 1;
    half4 Specular = tex2D(_GBuffer1, uv);
...
{
	half2 uv = IN.uv.xy; 
    half2 velocity = tex2D(_MotionVectorTexture, uv);
...
half4 CombienGTAO_frag(PixelInput IN) : SV_Target
{
	...
	{
        half3 Albedo = tex2D(_GBuffer0, uv);
		GTAO = MultiBounce(GTAO, Albedo);
	}

    half3 RelfectionColor = tex2D(urp_ReflProbes_Atlas, uv).rgb;
	//half3 SceneColor = GTAO * (tex2D(_AO_Scene_Color, uv).xyz - RelfectionColor);
    half3 SceneColor = GTAO * (tex2D(_AO_Scene_Color, uv).xyz);
	RelfectionColor *= GTRO;
	
    return half4(SceneColor, 1);// + RelfectionColor, 1);
}
