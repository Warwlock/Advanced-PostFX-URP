...
sampler2D _CameraNormalsTexture, _GBuffer0, _GBuffer1, _GBuffer2, _MotionVectorTexture, urp_ReflProbes_Atlas;
...
inline half3 GetNormal(half2 uv)
{
    ...
    half3 Normal = tex2D(_GBuffer2, uv).rgb * 2 - 1;
...
half4 GTAO(half2 uv, int NumCircle, int NumSlice, inout half Depth)
{
	...
	{
		angle = (i + noiseDirection + _AO_TemporalDirections) * (PI / (half)NumCircle);
		...
		cos_n = clamp(dot(normalize(projectedNormal), viewDir), 0, 1);
